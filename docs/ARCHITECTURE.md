...
- **Rate limiting:** `router = APIRouter(..., dependencies=[Depends(enforce_ai_rate_limit)])` — מגביל לפי IP (חלון-זמן קבוע, מונה בזיכרון תהליך, `services/rate_limit.py`), כדי להגן על מפתח ה-API מפני הצפה.

> **סטייה שהתגלתה מול docs/README.md §6:** README מתעד את `ai.py` כ"authenticated" ב-RBAC, אך בפועל **אין כלל `Depends(require_roles(...))` או `Depends(get_current_user)`** על אף אחד משלושת ה-endpoints — הבדיקה היחידה היא rate-limit לפי IP. שלושת ה-endpoints פתוחים לכל קורא (גם לא-מחובר), בכפוף למכסת הקצב. תוקן במטריצת ההרשאות (ROLES_MATRIX.md) בהתאם למה שנמצא בפועל בקוד.
