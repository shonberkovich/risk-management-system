...
| `risk_profiles.py` (`/api/properties/{id}/risk-profile`) | יצירה/עדכון סקר סיכונים (Asset_Risk_Profiles) — יחס 1:1 לנכס: POST פעם ראשונה בלבד (409 אם כבר קיים), PUT לעדכון (404 אם עוד אין); create/update מצליבים אוטומטית מול מאגר החומ"ס (`app/integrations/environmental.py`) ומעדכנים `near_hazmat_site` + עונש חד-פעמי ל-`fire_risk_score` (מוגבל לטווח 1-5) — מסתלסל בשקט אם האינטגרציה לא זמינה | RISK_MANAGER/PROPERTY_MANAGER/ADMIN |
| `incidents.py` (`/api/incidents`) | דיווח אירוע, טיוטה→הגשה, סטטוס, drill-down מאוחד; אירוע CRITICAL פותח אוטומטית משימת מיטיגציה + כרטיס ERP מדומה + התראת Push/SMS/Email מדומה לנמענים פעילים; `POST /api/incidents/check-seismic-activity` — הפעלה ידנית של `check_seismic_activity` (קורא ל-`app/integrations/seismology.py`, פותח טיוטת אירוע `STRUCTURAL_FAILURE` לכל נכס ברדיוס ההרגשה של רעידת אדמה משמעותית; אין cron אמיתי בפרויקט קורס זה) | תלוי endpoint — ראו הקוד; `check-seismic-activity`: RISK_MANAGER/ADMIN |
...
- **`backend/app/integrations/erp.py` נשארת מסומלת במלואה** — אין credentials/API אמיתי ל-ERP (SAP/Priority) בסביבה זו; המודול כתוב כך שקל להחליף בקריאת HTTP אמיתית ביום שיהיו פרטי חיבור. **עדכון (TODO_SPEC.md §12):** חמישה חיבורים אחרים **כן** בוצעו כקריאות HTTP אמיתיות לשירותים ציבוריים/חינמיים שאינם דורשים מפתח — `gis.py`'s `reverse_geocode` (OSM Nominatim), `economics.py`'s `fetch_boi_market_data` (Bank of Israel PublicApi — שערי חליפין ואינפלציה; מדד עלויות הבנייה המקורי ב-`fetch_index_series` נשאר מסומל), ושלושה מודולים חדשים לגמרי: `home_front.py` (התראות פיקוד העורף), `seismology.py` (המכון הגיאולוגי), `environmental.py` (מפעלי חומ"ס מ-data.gov.il). כל חמשת אלה עוטפים כל קריאה יוצאת ב-`app/integrations/_http.py` המשותף — timeout קצר, ותרגום כל כשל רשת/JSON ל-degradation נקי (`status="unavailable"`) במקום 500, כדי שסביבת פיתוח ללא גישה לאינטרנט (כמו סביבת ה-sandbox שבה פותח שינוי זה) לא תפיל את השרת. בדיקות ה-pytest של חמשת אלה ממוקות (`monkeypatch`) ואינן תלויות בגישה חיה לאינטרנט. **עדכון נוסף:** `weather.py` הפכה לחיבור היברידי — אם `OPENWEATHERMAP_API_KEY` מוגדר ב-`.env` (מפתח חינמי, ללא צורך במנוי בתשלום), `fetch_weather_alerts` קוראת בפועל ל-OpenWeatherMap's Current Weather Data endpoint לכל נכס פעיל ומגזירה התראה (STORM/FLOOD_WARNING/HEATWAVE) מתנאי מזג האוויר האמיתיים המוחזרים (קוד סופה, מהירות רוח, כמות גשם, טמפרטורה) מול ספי קבועים; ללא מפתח, נשמרת ההתנהגות המסומלת המקורית (hash דטרמיניסטי לפי `(lat, lon, date)`). בדיקות ב-`tests/test_integrations_weather.py`, ממוקות כמו חמשת האחרים. **עדכון נוסף (מדדים כלכליים):** `economics.py`'s `fetch_index_series`/`fetch_replacement_value_updates` (מדד תשומות בנייה + מדד המחירים לצרכן, ומכאן המלצת עדכון שווי כינון) הפכו לחיבור אמיתי, **ללא מפתח**, מול ה-API הציבורי של הלשכה המרכזית לסטטיסטיקה (CBS, `api.cbs.gov.il/index/data/price`) — סדרה 120010 (מדד המחירים לצרכן) וסדרה 200010 (מדד מחירי תשומה בבנייה למגורים), שני מספרים אמיתיים ומתעדכנים. אם הקריאה ל-CBS נכשלת (רשת לא זמינה, מבנה תגובה השתנה), שתי הפונקציות נופלות חזרה להתנהגות המסומלת המקורית (`ECON-SIM`), כדי שהמסך תמיד יחזיר ערך תקין. בדיקות ב-`tests/test_integrations_economics_cbs.py`, ממוקות ואינן תלויות בגישה חיה לאינטרנט. **עדכון נוסף (תיקון `cpi_yoy_percent` ב-BOI market data):** התברר בבדיקה חיה שנתיב `GetInflation` המתועד ב-BOI's PublicApi לא באמת עובד — הוא תמיד מחזיר את דף ה-loader של הגנת ה-bot (Radware) של האתר (200 עם HTML, לא JSON), אף פעם לא תשובה אמיתית. `fetch_boi_market_data`'s `cpi_yoy_percent` row עודכן למקור מ-CBS's price-index API (אותה קריאה ל-CBS ששמשת גם את `fetch_index_series`, פשוט קורא שדה נוסף — `percentYear` — שה-CBS כבר מפרסם לכל חודש) במקום מ-BOI's GetInflation השבור. כל שורה בתשובת `boi-market-data` נושאת עכשיו `source_system` משלה (`BOI-PUBLIC-API` לשערי חליפין, `CBS-PUBLIC-API` ל-CPI), מוצג ב-`BoiMarketDataCard.tsx` כשהוא שונה מהמקור הכללי בתחתית הכרטיס.
