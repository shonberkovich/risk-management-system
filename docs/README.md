...
**המלצה לצעד הבא:** טיפול בפער ה"ניתוח רב-שנתי" — `calculate_loss_ratio` ב-[`kpi.py`](../backend/app/services/kpi.py) כבר מקבל פרמטר `year` אך תמיד נקרא עם ברירת המחדל (שנה נוכחית), כך שיחס הנזקים המוצג בדשבורד מתעלם לגמרי משנת 2025. דורש אנדפוינט טרנד חדש (למשל `GET /api/analytics/loss-ratio-trend` שמחזיר יחס נזקים לפי שנה) וגרף מגמה קטן בדשבורד או במסך הדוחות — שינוי מתון בקאנד לצד תוספת UI ממוקדת.
