...
**המלצה לצעד הבא:** טיפול בפער "ייצוא דוחות" — אין שום יכולת ייצוא בקוד כיום. הצעד המעשי ביותר: כפתור "ייצוא ל-Excel" במסך `/claims` (טבלת המעקב אחר תביעות, [`ClaimsTable.tsx`](../frontend/src/components/ClaimsTable.tsx)) שמייצא את הנתונים המוצגים (כולל שדה "שולם בפועל" שכבר קיים) לקובץ `.xlsx` — ניתן לממש צד-קליינט בלבד (למשל עם ספריית `xlsx`/`sheetjs`) ללא שינוי בקאנד כלל, בדומה לדפוס של מטריצת הסיכונים האינטראקטיבית.
