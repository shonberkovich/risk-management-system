...
**המלצה לצעד הבא:** טיפול בפער "ייצוא דוחות PDF" — הוספת ייצוא PDF (למשל `reportlab` בבקאנד או `jsPDF` בפרונטאנד) ל"דוח הנהלה" מסכם (KPIs עיקריים + טבלת תביעות), בדומה בעקרון לייצוא ה-Excel הקיים.
