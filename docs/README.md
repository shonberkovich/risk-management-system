...
- **ייצוא דוחות PDF:** ייצוא ה-Excel מטבלת התביעות מומש (ראו למעלה), אך ייצוא PDF (למשל "דוח הנהלה" מסכם) עדיין לא קיים בקוד (לא reportlab, לא jsPDF).
...
**המלצה לצעד הבא:** טיפול בפער "התראות סף" — הוספת מנגנון בסיסי שמזהה כאשר חשיפה גיאוגרפית (MFL) או ריכוז אירועים בנכס חוצים סף מוגדר, ומציג התראה בדשבורד (למשל `GET /api/analytics/alerts` חדש + באנר/כרטיס התראות ב-`Dashboard.tsx`) — ללא צורך ב-push/SMS אמיתי, רק חישוב וסימון ב-UI.
