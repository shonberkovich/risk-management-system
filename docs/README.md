...
**המלצה לצעד הבא:** הפיכת מטריצת הסיכונים לאינטראקטיבית — ה-API כבר מחזיר `property_ids` לכל תא (`/api/analytics/risk-matrix`), אז זו תוספת UI ממוקדת בלבד (`onClick` על תא ב-[`RiskMatrix.tsx`](../frontend/src/components/RiskMatrix.tsx) שמסנן טבלת נכסים/אירועים לפי הנכסים בתא שנבחר), ללא שינוי בקאנד. לחלופין, אפשר לטפל בפער ה-"ניתוח רב-שנתי" (יחס הנזקים כרגע מתעלם לגמרי משנת 2025) — אך זה דורש גם אנדפוינט טרנד חדש, כך שהמטריצה האינטראקטיבית היא הצעד הקטן והמיידי יותר.
