...
### [x] משימה 7: צד לקוח - מנגנון Human-in-the-loop ו"כפתורי קסם" (Contextual AI)
...
* **שלבי ביצוע (Checklist)**:
  - [x] שלב 1: פיתוח "כרטיס פעולה" (Action Card) בתוך ממשק הצ'אט שמופיע כאשר הסוכן מציע פעולה (לדוגמה: "האם ליצור משימת הפחתת סיכונים? [אשר] / [בטל]").
  - [x] שלב 2: כפתור אישור שמפעיל את בקשת ה-Mutation ב-React Query שמבצעת את הפעולה בפועל.
  - [x] שלב 3: במסך פרטי נכס, הוספת כפתור "נתח סיכונים באמצעות AI" שיפתח את הצ'אט ויזריק באופן אוטומטי את ההקשר (Context) של מזהה הנכס הנוכחי.
* **קריטריוני הצלחה (Acceptance Criteria)**: הסוכן שולח הצעה לפתיחת משימה; מוצג למשתמש כרטיס עם כפתור אישור; הלחיצה יוצרת משימה בפועל ומרעננת את מסך המשימות. לחיצה מתוך תיק נכס מעבירה לסוכן את הקשר הנכס המדויק מבלי שהמשתמש יצטרך לציין את מזהה הנכס.
  - ✅ בוצע ב-branch feature/ai-action-card-property-context: `ActionCard.tsx` (נוצר כבר במשימה 6 בסיס, הושלם כאן) מציג הצעת פעולה עם כפתורי "אשר וצור משימה"/"בטל"; אישור מפעיל `useMutation` שקוראת ל-`createMitigationTask` (POST `/api/mitigation-tasks` בפועל) ואז מסמנת את ה-`Agent_Actions_Log` כ-`confirmed` דרך `confirmAgentAction`, ומרעננת את `["mitigation-tasks"]` ב-React Query כך שמסך המשימות מתעדכן. ב-`frontend/src/pages/PropertyDetail.tsx` (אין `PropertyDetails.tsx` בפרויקט — קובץ זה הוא המקביל בפועל) נוסף כפתור "נתח סיכונים באמצעות AI" הקורא ל-`useAIAssistant().openWithProperty({propertyId, propertyName})`; `AIAssistant.tsx` מאזין לכך ומזריק אוטומטית הודעת ניתוח + קריאה ל-`proposeMitigationTask(propertyId, sessionId)` תחת אותו session — המשתמש לא מקליד/בוחר מזהה נכס בעצמו. כוסה ב-`PropertyDetail.economics.test.tsx` (בלוק "נתח סיכונים באמצעות AI").
