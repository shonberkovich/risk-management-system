...
==================================================
[x] משימה 10: הגדרות אבטחה, הרשאות ואופטימיזציה של פניות
...
[x] משימה 14: חתימות אישיות וארגוניות למשתמשים (Email Signatures)
...
✅ בוצע ב-branch `feature/email-signatures`: `User.signature` (UnicodeText, מיגרציה `8594395c58b2`), מסונן דרך אותה `sanitize_body_html` ממשימה 10. `PATCH /api/users/{id}/signature` — 403 (לא 404, כי user_id אינו סודי) לכל מי שאינו הבעלים, כולל ADMIN. עמוד `ProfileSettings.tsx` חדש (העמוד האישי הראשון במערכת). הזרקה אוטומטית ב-`EmailComposeModal.tsx` (גוף נבנה מחדש בכל פתיחה — מונע כפילות), בתגובה מוכנסת מעל הציטוט. שלב 5 האופציונלי מומש: חתימת ברירת מחדל (שם+תפקיד) לעובד חדש. 547 טסטים בבקאנד + 168 בפרונט עברו.

==================================================

[x] משימה 15: סיכום מיילים ארוכים וסיווג באמצעות Claude AI
...
✅ בוצע ב-branch `feature/email-ai-summarize`: `POST /api/emails/{id}/summarize` ו-`suggest-reply` ב-`routers/emails.py` (לא ב-`ai.py`, כדי לעשות שימוש חוזר ב-`_require_recipient_row` הקיים). `llm.py` מקבל רק את הודעות השרשור הספציפי (מסונן לפי מה שהמשתמש רשאי לראות), נבדק בטסט ייעודי עם "marker" בשרשור לא קשור. 404 (לא-משתתף) נבדק לפני 503 (אין ANTHROPIC_API_KEY), תואם לתבנית הקיימת ב-`ai.py`. פרונט: כפתור ✨ לסיכום ו"הצע תשובה" שפותח את חלון הכתיבה במצב תגובה עם טיוטה לעריכה. 556 טסטים בבקאנד + 173 בפרונט עברו.
